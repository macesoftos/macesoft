...
function blockEmailHtml(block, origin) {
  ...
  if (block.type === "logo") {
    return `<div style="${style}font-family:Georgia,serif;font-size:34px;letter-spacing:.13em">${escapeEmailHtml(block.content || "MACE")}</div>`;
  }
  if (block.type === "image") {
  ...
  if (block.type === "heading") {
    return `<h1 style="${style}margin:0;font-family:${block.fontFamily === "Georgia" ? "Georgia,serif" : "Arial,sans-serif"};font-size:${fontSize}px;line-height:1.2">${textLines(block.content)}</h1>`;
  }
  if (block.type === "text") {
    return `<div style="${style}font-family:Arial,sans-serif;font-size:${fontSize}px;line-height:1.6">${textLines(block.content)}</div>`;
  }
  if (block.type === "button") {
    const link = safeEmailUrl(block.link, origin) || "#";
    return `<div style="${style}"><a href="${escapeEmailHtml(link)}" target="_blank" style="display:inline-block;padding:13px 28px;border-radius:6px;background:${safeColor(block.background, "#4a2d1c")};color:#fff;font-family:Arial,sans-serif;font-size:15px;text-decoration:none">${escapeEmailHtml(block.content)}</a></div>`;
  }
  ...
  if (block.type === "social") {
    const link = safeEmailUrl(block.link, origin) || "#";
    return `<div style="${style}font-family:Arial,sans-serif"><a href="${escapeEmailHtml(link)}" target="_blank" style="color:${color}">${escapeEmailHtml(block.content)}</a></div>`;
  }
...
export function buildVisualEmailHtml(draft, settings = {}, origin = "https://app.macebydrmace.com") {
  ...
  const content = blocks.map((block) => blockEmailHtml(block, origin)).join("\n");
  return `<!doctype html>
<html>
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f4f1ed"><tr><td align="center" style="padding:28px 12px">
    <table role="presentation" class="mace-email" width="640" cellpadding="0" cellspacing="0" style="width:640px;max-width:100%;background:#fff">
      <tr><td>${content}</td></tr>
      <tr><td style="padding:22px 28px;border-top:1px solid #ddd4c9;text-align:center;font-family:Arial,sans-serif;font-size:11px;line-height:1.6;color:#70675f"><strong>${escapeEmailHtml(company)}</strong><br>Davao City, Philippines<br><a href="#unsubscribe" style="color:#4a3324">Unsubscribe</a></td></tr>
    </table>
  </td></tr></table>
</body>
</html>`;
