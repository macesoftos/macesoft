...
function RenderedBlock({ block, preview, style }) {
  ...
  if (block.type === "image") {
    const ratio = { "1:1": "1 / 1", "4:3": "4 / 3", "3:2": "3 / 2", "16:9": "16 / 9", "2:1": "2 / 1" }[block.aspectRatio];
    return <div className="marketing-email-image" style={{ ...style, background: block.background, textAlign: block.align }}><img src={block.src || "/brand/result-1.jpg"} alt={block.decorative ? "" : block.alt || ""} style={{ aspectRatio: ratio || "auto", border: `${block.borderWidth || 0}px solid ${block.borderColor || "transparent"}`, borderRadius: block.borderRadius || 0, marginLeft: block.align === "right" ? "auto" : block.align === "center" ? "auto" : 0, marginRight: block.align === "left" ? "auto" : block.align === "center" ? "auto" : 0, maxWidth: block.maxWidth || "100%", objectFit: block.crop || "cover", objectPosition: `${block.focalX ?? 50}% ${block.focalY ?? 50}%`, transform: `scale(${Math.max(100, Number(block.zoom || 100)) / 100})`, transformOrigin: `${block.focalX ?? 50}% ${block.focalY ?? 50}%`, width: `${mobile ? block.responsive?.mobileWidth ?? block.mobileWidth ?? block.width ?? 100 : block.width || 100}%` }} />{block.caption && <small>{block.caption}</small>}</div>;
  }
...
function MarketingImageControl({ block, loadMedia, notify, onUploadStateChange, updateBlock, uploadImage }) {
  ...
  return (
    <section className="marketing-image-control">
      ...
      >
        {block.src && !imageFailed ? <img onError={() => setImageFailed(true)} src={block.src} alt="Selected image preview" style={{ objectFit: block.crop || "cover", objectPosition: `${block.focalX ?? 50}% ${block.focalY ?? 50}%`, transform: `scale(${Math.max(100, Number(block.zoom || 100)) / 100})` }} /> : <span><ImageIcon size={24} /><small>{imageFailed ? "Image could not be loaded" : dragging ? "Drop to upload" : "No image selected"}</small></span>}
      ...
      <small>Drag and drop or choose a JPG, PNG or WebP · maximum 3 MB. Failed replacements keep the current image.</small>
