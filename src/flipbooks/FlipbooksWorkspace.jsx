    ...
    <div className="flipbook-workspace-shell">
      <aside className="flipbook-subnav">
        ...
        <div className="flipbook-subnav-footer">
          <button className="flipbook-exit-workspace" type="button" onClick={onExit}><ArrowLeft size={16} /><span>Back to ZenshoTech</span></button>
          <div className="flipbook-workspace-account"><span>{initials}</span><div><strong>{session?.name || "ZenshoTech User"}</strong><small>{session?.role || "Account"}</small></div></div>
        </div>
      </aside>
      ...
      <div className="flipbook-modal share-modal">
        ...
        {book.status !== "Published" ? (
          ...
          <>
            ...
            <div className="flipbook-share-status"><Globe2 size={17} /><div><strong>{book.passwordProtected ? "Password protected" : "Anyone with the link"}</strong><span>No ZenshoTech account is required.</span></div></div>
...
export function PublicFlipbookViewer({ token }) {
  ...
  return (
    <main className="public-flipbook-viewer" style={{ "--public-background": branding.viewerBackground || "#f4f1ed" }}>
      <header><div className="public-flipbook-brand">{branding.logo ? <img src={branding.logo} alt="" /> : <BookOpen size={22} />}<strong>{branding.businessName || "ZenshoTech"}</strong></div><div className="public-flipbook-title"><strong>{book.title}</strong><span>{book.pageCount} pages</span></div><div>{book.allowDownload && <button type="button" onClick={() => void downloadPublicFlipbook(token, accessToken, book.title)}><Download size={17} /><span>Download PDF</span></button>}</div></header>
