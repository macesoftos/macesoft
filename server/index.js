...
async function deliverLeadAutomationRun(automation, lead, scheduledAt) {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const managers = await tx.account.findMany({
        where: { organizationId: automation.organizationId, status: "Active", role: { in: ["Owner", "Business Owner", "Super Admin"] } },
        select: { id: true },
      });
      if (managers.length) {
        await createAppNotification(tx, {
...
async function processLeadAutomation(automation) {
  ...
  if (!leadAutomationProviderReady(automation.channel)) return { evaluated: 0, sent: 0, failed: 0, skipped: 0, waitingForProvider: true };
...
app.get("/api/leads/automations", asyncRoute(async (request, response) => {
  ...
  response.json({
    ...
    providers: { email: emailReady(), sms: smsReady(), dryRun: envFlag(process.env.LEAD_AUTOMATION_DRY_RUN) },
  });
}));
...
app.put("/api/leads/automations/:id", asyncRoute(async (request, response) => {
  ...
  const automation = await prisma.$transaction(async (tx) => {
    ...
    await writeAudit(tx, request, { area: "Leads", action: "Lead automation updated", subjectType: "LeadAutomation", subjectId: updated.id, details: `${updated.name} saved in ${updated.active ? "active" : "draft"} mode.`, beforeValues: current, afterValues: updated });
    return updated;
  });
...
app.get("/api/public-leads/config", asyncRoute(async (request, response) => {
  ...
  const [branding, serviceRows, staffRows, siblingForms] = await Promise.all([
  ...
  response.json({
    ...
    staff: staffRows
      .map((person) => ({ ...person, branches: parseJsonList(person.branches) }))
      .filter((person) => allowedBranchNames.has(person.branch) || person.branches.some((branch) => allowedBranchNames.has(branch))),
  });
}));
