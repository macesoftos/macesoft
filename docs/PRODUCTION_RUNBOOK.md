...
4. To enable invitations and password resets, configure and verify an authenticated SMTP mailbox with `SMTP_HOST`, `SMTP_FROM`, `SMTP_USER`, and `SMTP_PASS`. Without all four values the app remains available, but email delivery endpoints return a service-unavailable response. Send a reset email through the deployed login screen before relying on password recovery.
5. For a new database, run `pnpm exec prisma migrate deploy`. The baseline migration creates the complete original schema and later migrations apply invitations, FaceTrack, uploads, branch isolation, password resets, and append-only audit protection.
6. For an existing database originally created with `prisma db push`, first take and verify a backup. Compare the live schema, then mark only migrations whose SQL is already represented with `prisma migrate resolve --applied <migration-name>`. Never mark an unapplied migration as applied.
7. Create the first Owner once with `BOOTSTRAP_OWNER_EMAIL`, `BOOTSTRAP_OWNER_NAME`, and a strong `BOOTSTRAP_OWNER_PASSWORD`, then run `pnpm bootstrap:owner`. The command refuses to run after any account exists. Never run the demo seed in production.
8. Deploy the container behind HTTPS. `/api/health/live` is the liveness probe and `/api/health/ready` is the database readiness probe.
