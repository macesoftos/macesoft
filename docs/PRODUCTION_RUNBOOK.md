...
4. Configure and verify the authenticated SMTP mailbox used for invitations and password resets. The app can start without SMTP while `REQUIRE_SMTP=false`; password-reset email remains unavailable. After adding `SMTP_HOST`, `SMTP_FROM`, `SMTP_USER`, and `SMTP_PASS`, set `REQUIRE_SMTP=true` and send a reset email through the deployed login screen before launch.
