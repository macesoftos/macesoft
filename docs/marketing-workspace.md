...
- The controlled builder persists in-progress content in browser storage and saves the compatible campaign fields through the current `campaigns` resource.
...
2. Persist email designs as structured, versioned block JSON in a Marketing template/design model. Keep rendered HTML server-generated from an allowlisted component schema.
